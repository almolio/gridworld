...
class Environment(object):
    ...
    def get_obs(self):
        """Compute the set of observations sent as input to the policy.
        TODO: To be expanded, this is just an example. """
        # transition = self.get_transition()

        # distance from actor and target to closest item
        self.item_locs += [3,2]
        self.item_locs += [1,0]
        self.item_locs += [3,4]
        ...
        if self.item_locs:
            items = []
            for i in range(int(len(self.item_locs)/2)):
                items.append([self.item_locs[2*i], self.item_locs[2*i+1]])
            items = np.array(items)
            dist_2_actor = np.linalg.norm(agent_loc - items, axis=1)
            dist_2_target = np.linalg.norm(target_loc - items, axis=1)
            closest_item_dist = np.min(dist_2_actor + dist_2_target)
            closest_item_id = np.argmin(dist_2_actor + dist_2_target)
            item_dist = items[closest_item_id]
        # if no item in the world send default value
        else:
            item_dist =  np.array([-1,-1])

        return [agent_loc[0], agent_loc[1], item_dist[0], item_dist[1]]